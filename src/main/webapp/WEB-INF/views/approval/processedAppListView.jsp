						...
						<div class="col-md-6 col-sm-12">
							...
							<nav aria-label="breadcrumb" role="navigation">
								<ol class="breadcrumb">
									...
									<li class="breadcrumb-item active" aria-current="page">결재 완료 문서함</li>
								</ol>
							</nav>
						</div>
					</div>
							...
							<c:forEach items="${ appList }" var="app">
								<tr>
									...
									<c:choose>
		             			 	...
		             			 	<!-- 문서구분 -->
		             			 	<c:choose>
		             			 		<c:when test="${ sessionScope.loginUser.empId  eq app.empId }">
		             			 			<td>기안</td>
		             			 		</c:when>
		             			 		<c:when test="${ sessionScope.loginUser.empId  eq app.appMid }">
		             			 			<td>요청</td>
		             			 		</c:when>
		             			 		<c:otherwise>
		             			 			<td>요청</td>
                ...
                <ul class="pagination">
                	<c:choose>
                		<c:when test="${ pi.currentPage ne 1 }">
                			<li class="page-item"><a class="page-link" href="apped.ap?currentPage=${ pi.currentPage-1 }">이전</a></li>
                		</c:when>
                		<c:otherwise>
                    ...
                    <c:forEach begin="${ pi.startPage }" end="${ pi.endPage }" var="p">
                    	<c:choose>
	                		<c:when test="${ pi.currentPage ne p }">
                    			<li class="page-item"><a class="page-link" href="apped.ap?currentPage=${ p }">${ p }</a></li>
	                		</c:when>
	                		<c:otherwise>
                    ...
                    <c:choose>
                		<c:when test="${ pi.currentPage ne pi.maxPage }">
                			<li class="page-item"><a class="page-link" href="apped.ap?currentPage=${ pi.currentPage+1 }">다음</a></li>
                		</c:when>
                		<c:otherwise>
